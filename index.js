...
/**
 * Get an instance of the Web Crypto API, if available.
 *
 * @private
 * @return {Crypto|null}
 */
...
/**
 * An Error thrown if the m3api-oauth2/requireCrypto option is set,
 * but the Web Crypto API is unexpectedly not available.
 */
class NoWebCryptoApiError extends Error {
	...
	constructor( ...params ) {
		super(
			'm3api-oauth2: The Web Crypto API is unavailable, ' +
				'but required because the m3api-oauth2/requireCrypto request option was set.',
			...params,
		);

		if ( Error.captureStackTrace ) {
			Error.captureStackTrace( this, NoWebCryptoApiError );
		}

		this.name = 'NoWebCryptoApiError';
	}
 ...
 * Defaults to true.
 * @property {boolean} ['m3api-oauth2/requireCrypto']
 * Whether to require the Web Crypto API to generate and hash the PKCE code verifier.
 ...
 * If true, m3api-oauth2 will instead throw a {@link NoWebCryptoApiError}
...
Object.assign( DEFAULT_OPTIONS, {
	...
	'm3api-oauth2/requireCrypto': false,
...
async function handleInvalidAuthorizationError( session, params, options ) {
	...
	// eslint-disable-next-line no-undef -- m3api/add-performance-global.js should ensure performance exists
...
async function initOAuthSession( session, options = {} ) {
	const {
		...
		'm3api-oauth2/requireCrypto': requireCrypto,
	} = {
		...DEFAULT_OPTIONS,
		...session.defaultOptions,
		...options,
	};
	const codeVerifier = await newCodeVerifier( requireCrypto );
	...
	const params = new URLSearchParams( {
		...
		...await getCodeChallenge( codeVerifier, requireCrypto ),
	} );
 ...
 *
 * @private
 * @param {boolean} requireCrypto Whether to require the Web Crypto API
 * or allow falling back to Math.random().
 * @return {string}
 */
async function newCodeVerifier( requireCrypto ) {
 ...
 * @param {boolean} requireCrypto Whether to require the Web Crypto API
 * or allow falling back to the unhashed code verifier as code challenge.
 * @return {Object} To be mixed into the authorization request parameters.
 */
async function getCodeChallenge( codeVerifier, requireCrypto ) {
	const crypto = await webCrypto();
	if ( crypto !== null ) {
